...
const MainHREventsPage = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/main-hr/events?page=${currentPage}&search=${searchTerm}`);
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      <div className="flex items-center justify-between mb-4">
        <h1 className="hidden md:block text-lg font-semibold">Events Management (All Branches)</h1>
      ...
      <Pagination page={currentPage} count={totalPages * ITEM_PER_PAGE} />
    </div>
  );
};

export default MainHREventsPage;
