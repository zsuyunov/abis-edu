...
const MainHRDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const response = await fetch("/api/main-hr/dashboard");
      ...
      <div className="w-full lg:w-2/3 flex flex-col gap-8">
        ...
        <div className="flex gap-4 justify-between flex-wrap">
          <UserCardClient 
            ...
            subtitle="Total Teachers (All Branches)"
          />
          <UserCardClient 
            type="student" 
            count={dashboardData?.studentCount || 0}
            subtitle="Total Students (All Branches)"
          />
          <UserCardClient 
            type="parent" 
            count={dashboardData?.parentCount || 0}
            subtitle="Total Parents (All Branches)"
          />
          <UserCardClient 
            type="staff" 
            count={dashboardData?.staffCount || 0}
            subtitle="Total Staff (All Branches)"
          />
        </div>
        {/* MIDDLE CHARTS */}
