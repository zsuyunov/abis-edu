...
import { useChiefMessages } from "@/hooks/useChief";

export default function ChiefMessagesPage() {
  const [activeTab, setActiveTab] = useState("inbox");
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const { data, isLoading, error } = useChiefMessages(activeTab, page, search);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/chief/messages", { 
        method: "POST", 
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="flex gap-2">
                {msg.status === "SENT" && activeTab === "inbox" && (
                  <button 
                    onClick={async () => { 
                      await fetch(`/api/chief/messages/${msg.id}/read`, { method: 'POST' }); 
                      window.location.reload(); 
                    }} 
                ...
                <button 
                  onClick={async () => { 
                    await fetch(`/api/chief/messages?id=${msg.id}`, { method: 'DELETE' }); 
                    window.location.reload(); 
                  }} 
