...
import { useChiefAnnouncements } from "@/hooks/useChief";

export default function ChiefAnnouncementsPage() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const { data, isLoading, error } = useChiefAnnouncements(page, search);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/chief/announcements", { 
        method: "POST", 
        ...
        {(data?.data || []).map((ann: any) => (
          <div key={ann.id} className="border rounded p-4">
            <div className="flex justify-between items-start">
              ...
              <button 
                onClick={async () => { 
                  await fetch(`/api/chief/announcements?id=${ann.id}`, { method: 'DELETE' }); 
                  window.location.reload(); 
                }} 
