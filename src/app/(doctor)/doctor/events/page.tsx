...
import { useDoctorEvents } from "@/hooks/useDoctor";

export default function DoctorEventsPage() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const { data, isLoading, error } = useDoctorEvents(page, search);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/doctor/events", { 
        method: "POST", 
        ...
        {(data?.data || []).map((ev: any) => (
          <div key={ev.id} className="border rounded p-4">
            <div className="flex justify-between items-start">
              ...
              <button 
                onClick={async () => { 
                  await fetch(`/api/doctor/events?id=${ev.id}`, { method: 'DELETE' }); 
                  window.location.reload(); 
                }} 
