    ...
    >
      <td className="flex items-center gap-4 p-4">
        ...
        <div className="flex flex-col">
          <h3 className="font-semibold">{item.shortName}</h3>
          <p className="text-xs text-gray-500">{item.legalName}</p>
        </div>
      </td>
      <td className="hidden md:table-cell">{item.stir}</td>
        ...
        switch (key) {
          case "search":
            query.OR = [
              ...
              { stir: { contains: value, mode: "insensitive" } },
              { phone: { contains: value, mode: "insensitive" } },
            ];
            break;
          case "status":
            query.status = value as BranchStatus;
            break;
          default:
            break;
        }
