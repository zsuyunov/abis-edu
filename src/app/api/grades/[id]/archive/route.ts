...
import { archiveGrade } from "@/lib/actions";
...
export async function POST(
  ...
  try {
    ...
    const result = await archiveGrade(params.id, comment, currentUserId);
    
    if (result.success) {
      return NextResponse.json({ message: "Grade archived successfully" });
    } else {
      return NextResponse.json(
