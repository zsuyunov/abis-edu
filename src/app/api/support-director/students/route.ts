    ...
    const [students, count] = await Promise.all([
      prisma.student.findMany({
        ...
        include: {
          ...
          // parents are now via StudentParent join; omit legacy parent include
        },
        take: ITEM_PER_PAGE,
...
export async function POST(request: NextRequest) {
  try {
    const headersList = headers();
    const role = headersList.get("x-user-role");
    const branchIdHeader = headers.get("x-branch-id");
    if (role !== "support_director") return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    const branchId = Number(branchIdHeader);
    const body = await request.json();
    const row = await prisma.student.create({ data: { ...body, branchId } });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { id, ...data } = body;
    const updated = await prisma.student.updateMany({ where: { id, branchId }, data });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deleted = await prisma.student.updateMany({ where: { id, branchId }, data: { archivedAt: new Date() } });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const restored = await prisma.student.updateMany({ where: { id, branchId }, data: { archivedAt: null } });
