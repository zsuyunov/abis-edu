...
export async function GET(request: NextRequest) {
  try {
    ...
    const [rows, count] = await Promise.all([
      prisma.homework.findMany({ where, include: { subject: true, class: true }, take: ITEM_PER_PAGE, skip: ITEM_PER_PAGE * (page - 1), orderBy: { createdAt: "desc" } }),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const row = await prisma.homework.create({ data: { ...body, branchId } });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const updated = await prisma.homework.updateMany({ where: { id, branchId }, data });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const archived = await prisma.homework.updateMany({ where: { id, branchId }, data: { archivedAt: new Date() } });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const restored = await prisma.homework.updateMany({ where: { id, branchId }, data: { archivedAt: null } });
