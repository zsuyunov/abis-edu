...
export async function GET(request: NextRequest) {
  try {
    ...
    const [rows, count] = await Promise.all([
      prisma.event.findMany({ where, take: ITEM_PER_PAGE, skip: ITEM_PER_PAGE * (page - 1), orderBy: { startTime: "desc" } }),
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!title || !startTime) return NextResponse.json({ error: "Title and start time required" }, { status: 400 });

    const row = await prisma.event.create({ data: { title, description, startTime: new Date(startTime), endTime: endTime ? new Date(endTime) : null, branchId } });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const updated = await prisma.event.updateMany({ where: { id, OR: [{ branchId }, { branchId: null }] }, data: { title, description, startTime: startTime? new Date(startTime) : undefined, endTime: endTime? new Date(endTime): undefined } });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!id) return NextResponse.json({ error: "id required" }, { status: 400 });
    const deleted = await prisma.event.deleteMany({ where: { id, OR: [{ branchId }, { branchId: null }] } });
