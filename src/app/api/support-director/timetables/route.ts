...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ timetables: rows, totalPages: Math.ceil(count / ITEM_PER_PAGE), currentPage: page, totalItems: count });
  } catch (e) {
    console.error(e);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { subjectId, teacherId, classId, dayOfWeek, startTime, endTime } = body;
    const row = await prisma.timetable.create({ data: { subjectId, teacherId, classId, dayOfWeek, startTime, endTime, branchId } });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const updated = await prisma.timetable.updateMany({ where: { id, branchId }, data });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const archived = await prisma.timetable.updateMany({ where: { id, branchId }, data: { archivedAt: new Date() } });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const restored = await prisma.timetable.updateMany({ where: { id, branchId }, data: { archivedAt: null } });
