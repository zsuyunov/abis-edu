...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userId || userRole !== "support_director" || !branchId) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    // Get meal plans in this branch that need support director approval
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (status === "REJECTED") {
      ...
      // Check if doctor has also approved
      const doctorApproval = await prisma.mealApproval.findFirst({
        where: {
          mealPlanId,
          approverType: "DOCTOR",
          status: "APPROVED",
