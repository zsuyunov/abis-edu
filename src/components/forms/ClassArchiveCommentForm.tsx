...
import { archiveClass, restoreClass, deleteClass } from "@/lib/actions";
import { useRouter } from "next/navigation";
import { toast } from "react-toastify";

const ClassArchiveCommentForm = ({
  classId,
  className,
  action,
  currentUserId,
  ...
  useEffect(() => {
    if (state.success) {
      ...
      toast(`Class ${className} has been ${actionPastTense}!`);
      setOpen(false);
      router.refresh();
    }
  ...
  return (
    <form className="flex flex-col gap-6" onSubmit={onSubmit}>
      <h1 className="text-xl font-semibold">
        {action === "ARCHIVE" && "Archive Class"}
        {action === "RESTORE" && "Restore Class"}
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            ...
            <div className={`mt-2 text-sm ${
              ...
              {action === "ARCHIVE" && "This will make the class inactive and hide it from active lists."}
              {action === "RESTORE" && "This will make the class active again and show it in active lists."}
              {action === "DELETE" && "This action cannot be undone. The class and all related data will be permanently removed."}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <textarea
          ...
          placeholder={`Please provide a reason for ${action.toLowerCase()}ing this class (minimum 10 characters)...`}
        />
...
export default ClassArchiveCommentForm;
