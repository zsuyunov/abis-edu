...
const SmallResetPasswordModal = ({
  ...
  const [state, formAction] = useFormState(resetStudentPassword as any, {
    success: false,
    error: false,
  });
  ...
  useEffect(() => {
    if (state.success) {
      toast.success((state as any).message || `Password reset for ${studentName}!`);
      setOpen(false);
      reset();
      router.refresh();
    }
...
export default SmallResetPasswordModal;
