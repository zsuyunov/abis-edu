...
const DoctorSettings = ({ userId }: DoctorSettingsProps) => {
  ...
  const { data: settings, isLoading } = useQuery<DoctorSettings>({
    queryKey: ["doctor-settings", userId],
    queryFn: async () => {
      const response = await fetch(`/api/doctor/settings/${userId}`);
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: async (updatedSettings: Partial<DoctorSettings>) => {
      const response = await fetch(`/api/doctor/settings/${userId}`, {
        method: "PUT",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["doctor-settings", userId] });
    },
  });

  const updatePasswordMutation = useMutation({
    mutationFn: async (passwordData: { newPassword: string; confirmPassword: string }) => {
      const response = await fetch(`/api/doctor/change-password/${userId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSettingChange = (category: keyof DoctorSettings, key: string, value: any) => {
    if (!settings) return;
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Medical Settings</h1>
          <p className="text-gray-600">Manage your medical panel preferences</p>
        </div>
      </div>
        ...
        <div className="p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">Get notified when meal plans need medical approval</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">Receive urgent health-related notifications</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
          ...
          {activeTab === "preferences" && (
            <div className="space-y-6">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Medical Preferences</h3>
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div>
                    <h4 className="font-medium text-gray-900">Auto-Approval Reminders</h4>
                    <p className="text-sm text-gray-600">Get reminded about pending meal approvals before auto-approval</p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-sm text-gray-600 mb-3">Control who can see your medical profile</p>
                    <select
