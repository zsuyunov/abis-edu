...
const StudentAttendanceAnalytics = ({ studentId }: StudentAttendanceAnalyticsProps) => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Period Selector */}
      <div className="bg-gray-100 rounded-full p-1 flex flex-wrap gap-1 sm:gap-0">
        {(["7days", "4weeks", "12weeks", "year"] as const).map((period) => (
          <button
            key={period}
            onClick={() => setSelectedPeriod(period)}
            className={`px-3 sm:px-6 py-2 rounded-full text-xs sm:text-sm font-medium transition-all flex-1 sm:flex-none ${
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-lg border">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6 mb-6 sm:mb-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <p className="text-lg sm:text-2xl font-bold text-gray-900">{stats.present}</p>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <p className="text-lg sm:text-2xl font-bold text-gray-900">{stats.late}</p>
              <p className="text-xs sm:text-sm text-gray-600">Lates</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <p className="text-lg sm:text-2xl font-bold text-gray-900">{stats.absent}</p>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <p className="text-lg sm:text-2xl font-bold text-gray-900">{stats.excused}</p>
              <p className="text-xs sm:text-sm text-gray-600">Excused</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 col-span-2 sm:col-span-1">
            ...
            <div>
              <p className="text-lg sm:text-2xl font-bold text-gray-900">{stats.total}h</p>
        ...
        <div className="flex justify-center">
          <div className="relative w-48 h-48 sm:w-64 sm:h-64">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center bg-white/90 backdrop-blur-sm rounded-full w-24 h-24 sm:w-32 sm:h-32 flex flex-col items-center justify-center shadow-lg">
                <div className="text-2xl sm:text-4xl font-bold bg-gradient-to-r from-green-600 to-blue-600 bg-clip-text text-transparent">
                  {Math.round(stats.percentage)}%
                </div>
                <div className="text-xs sm:text-sm text-gray-600 font-medium">Attendance</div>
                <div className="text-[10px] sm:text-xs text-gray-500">Tap to view</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-lg border">
        <div className="mb-4">
          <h3 className="text-base sm:text-lg font-semibold text-gray-900">Attendance Trends</h3>
          <p className="text-xs sm:text-sm text-gray-600">Weekly attendance percentage over time</p>
        </div>
          ...
          {barData.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <h4 className="text-base sm:text-lg font-medium text-gray-900 mb-2">No Data Available</h4>
                <p className="text-sm text-gray-500">Attendance trends will appear here once you have more data</p>
              </div>
            </div>
          )}
        </div>
