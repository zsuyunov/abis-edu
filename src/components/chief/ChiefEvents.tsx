...
interface ChiefEventsProps {
  userId: string;
}

const ChiefEvents = ({ userId }: ChiefEventsProps) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [filterType, setFilterType] = useState("all");
  const [showCreateModal, setShowCreateModal] = useState(false);

  const { data: events, isLoading } = useQuery<Event[]>({
    queryKey: ["chief-events", userId, searchTerm, filterType],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/chief/events?${params}`);
