...
import { z } from "zod";
import { resetTeacherPassword } from "@/lib/actions";
...
const SmallTeacherResetPasswordModal = ({
  userId,
  userName,
  currentUserId,
}: {
  ...
  const [state, formAction] = useFormState(resetTeacherPassword, {
    success: false,
    error: false,
    message: '',
  });
  ...
  useEffect(() => {
    if (state.success) {
      toast.success(state.message || `Password reset for ${userName}!`);
      setOpen(false);
      reset();
      router.refresh();
    }
...
export default SmallTeacherResetPasswordModal;
